...
function filterLowPowerIntelPcs(pcs: ServerPcWithCpuSpec[]) {
  return pcs.filter((pc) => {
    const cpu = (pc.cpu || '').toLowerCase()
    return cpu.includes('n95') || cpu.includes('n100') || cpu.includes('n150')
  })
}

export default async function Article47Page() {
  const pcs = filterLowPowerIntelPcs(await fetchPcList('cost_performance'))
  ...
  return (
    <PcDbArticle
      ...
      criteria={[
        ...
        'N95/N100/N150の違いだけでなく、価格、重量、画面サイズ、バッテリーを見る',
        '動画編集、3Dゲーム、多数アプリ同時利用ならCore/Ryzen系も比較する',
      ]}
      dataAngleTitle="このサイト特有の見方"
      dataAngle="SpecsyではAmazon内のPCをPC-DB化し、N95/N100/N150搭載というCPU条件で抽出したうえで、価格、メモリ、SSD、画面サイズ、重量、推定駆動時間、用途別スコアを同じ表で比較できます。CPU名だけではなく、実際に売られている構成差を見られる点が強みです。"
      tableDescription="下表は、PC-DB内でN95、N100、N150搭載が確認できる候補を、コスパスコア順に並べたものです。CPU名だけでなく、メモリ、SSD、重量、画面サイズ、価格のバランスを確認してください。"
      faq={[
        {
          question: 'N95、N100、N150はどれを選ぶべきですか？',
          answer: '同じ価格帯ならN150やN100を優先しやすいですが、体感差はメモリ、SSD、冷却、バッテリーにも左右されます。CPU名だけでなく、16GB/512GB構成かどうかを優先して比較してください。',
        },
        {
          question: 'N95/N100/N150搭載PCはメインPCにできますか？',
